...
def parse_args():
    ...
    parser.add_argument('--input_folder', help='the frames path', default="./frames")
    parser.add_argument('--out_file', help='the dir to save results', default="./results.txt")
...
def getscore(model, video_id, img, list_det):
    ...
    cv2.imwrite(os.path.join('./crop_test/1', 'img_%06d.jpg'%(list_det[0])), roi)
...
if __name__ == '__main__':
    ...
    for i in range(len(videos)):
        ...
        # with open('./test.txt', 'a+') as f:
            for pred in preds:
                f.write(" he".join(str(i) for i in pred) + '\n')
